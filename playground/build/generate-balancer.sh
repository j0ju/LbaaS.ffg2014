...
mkfs.ext3 "$ROOT_P" -L balancer-root
eval "$(blkid "$ROOT_P" -o export)" && ROOT_UUID="$UUID"
eval "$(mkswap "$SWAP_P" -L balancer-swap -f)" && SWAP_UUID="$UUID"
...
[ -d "$MYPATH"/balancer.local ] && rsync -a "$MYPATH"/balancer.local "$MNT"

sed -i -e "s/rootfs-base/balancer/" $( grep "base-rootfs" -rl "$MNT"/etc )
